...
public class ChainLasherEnemy extends Enemy {
    private int attackCooldown = 0;

    public ChainLasherEnemy(Mob entity, WaveContext context, AbstractEnemyType type) {
        ...
        if (entity.getTarget() instanceof Player) {
            ...
            if (target != null && entity.hasLineOfSight(target)) {
                ...
                if (distance < 15 && attackCooldown <= 0) {
                    throwChain(target);
                    attackCooldown = 20; // 20 * 5 ticks = 100 ticks = 5 seconds
                }
            }
        }
    }

    private void throwChain(Player target) {
